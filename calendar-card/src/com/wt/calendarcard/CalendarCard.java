...
public class CalendarCard extends RelativeLayout {
    ...
    private List<Integer> dateChecked = new ArrayList<Integer>();
...
//						((CheckableLayout)v).toggle();
                        for (CheckableLayout c : cells) {
                            ((TextView) ((CheckableLayout) c).getChildAt(0)).setTypeface(Typeface.DEFAULT);
                        }

                        ((TextView) ((CheckableLayout) v).getChildAt(0)).setTextSize(20);

                        ((TextView) ((CheckableLayout) v).getChildAt(0)).setTypeface(Typeface.DEFAULT_BOLD);
    ...
    private void updateCells() {
        ...
        Log.e(TAG, "updateCells: "+firstDay );
        cal.set(Calendar.DAY_OF_MONTH, cal.getActualMaximum(Calendar.DAY_OF_MONTH));
        int lastDay = cal.get(Calendar.DAY_OF_MONTH) + 1;
        Log.e(TAG, "updateCells: "+lastDay );
        for (int i = firstDay; i < lastDay; i++) {
            ...
            //所有课程的集合 当有课的时候 可以点击 并且设置为checked
            if (dateChecked.contains(i)) {
